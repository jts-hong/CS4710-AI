...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # If ghost has been captured, initialize new Counter with only one entry corresponding to the jailed ghost
        if noisyDistance == None:
            self.beliefs = util.Counter()
            self.beliefs[self.getJailPosition()] = 1.0
            return

        # Otherwise, retrieve emission model and current position
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()

        # Iterate over all possible positions and update using B(X_t+1)=P(e|X_t+1)B(X_t)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for i in self.legalPositions:
            nPD = self.getPositionDistribution(self.setGhostPosition(gameState,i))
            for j,k in nPD.items():
                aP[j]+=(self.beliefs[i]*k)
        self.beliefs = aP
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            aP = util.Counter()
            cB = self.getBeliefDistribution()
            for i in self.legalPositions:
                d = util.manhattanDistance(pacmanPosition,i)
                aP[i]=emissionModel[d]*cB[i]
            aP.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for i in self.particles:
            ...
            rP.append(util.sample(newPosDist))
        self.particles= rP
        #util.raiseNotDefined()
