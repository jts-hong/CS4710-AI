...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for i in self.getLegalActions(state):
          if self.getQValue(state, i) == bv:
            ba.append(i)
        if not len(ba): return None
        else: return random.choice(ba)
        #util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        disc = self.discount
        a = self.alpha
        qva = self.getQValue(state, action)
        nv = self.getValue(nextState)
        

        nv = (1-a) * qva + a * (reward + disc * nv)
        self.qv[(state, action)] = nv
